...
public class MessageDialogFragmentV4 extends DialogFragment {
    ...
    public interface MessageDialogListener {
        void onMessageDialogResult(final MessageDialogFragmentV4 dialog, final int requestCode, 
    ...
    public static MessageDialogFragmentV4 showDialog(final FragmentActivity parent, final int requestCode,
                                                    final int title, final int message, final String... permissions) {
        final MessageDialogFragmentV4 dialog = newInstance(requestCode, title, message, permissions);
        final FragmentManager fm = parent.getSupportFragmentManager();
        dialog.show(fm, "MessageDialogFragmentV4");
        return dialog;
    }
    
    public static MessageDialogFragmentV4 newInstance(final int requestCode, final int title, final int message, 
    ...
    private void callOnMessageDialogResult(final int requestCode, final String[] permissions, 
        ...
        if (activity instanceof MessageDialogListener) {
            ((MessageDialogListener) activity).onMessageDialogResult(this, requestCode, permissions, result);
        }
    }
} 