...
public final class USBMonitor {
	...
	public synchronized void register() throws IllegalStateException {
		...
		if (mPermissionIntent == null) {
			...
			if (context != null) {
				// For Android 12+ (API 31+), we need FLAG_IMMUTABLE for security
				...
				// For Android 13+ (API 33+), we need to specify receiver export flag
		...
		if (useNewAPI && BuildCheck.isAndroid5()) {
				...
				if (TextUtils.isEmpty(serial)) {
						try {
								...
								// No permission yet, skip serial number
				...
				try {
						...
						// No permission yet, skip manufacturer name
						sb.append("no_permission");	sb.append("#");
				}
				...
				if (BuildCheck.isMarshmallow()) {
						try {
								...
								// No permission yet, skip version
								sb.append("no_permission");	sb.append("#");
						}
				}
		}
